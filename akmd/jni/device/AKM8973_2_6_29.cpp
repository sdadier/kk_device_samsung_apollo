...
#ifdef AKM8976
#include "linux-2.6.29/akm8976.h"
#else
...
AKM8973_2_6_29::AKM8973_2_6_29(int gain)
    : index(0), fixed_magnetometer_gain(0), magnetometer(0)
{
...
#ifdef AKM8976
    fd = open("/dev/akm8976_daemon", O_RDONLY);
#else
...
void AKM8973_2_6_29::measure() {
    ...
    temperature = (signed char) (35 + (120 - akm_data[1])*10/16);
    LOGI("t:%d x:%d y:%d z:%d", temperature, akm_data[2], akm_data[3], akm_data[4]);
	...
	mbuf[index] = Vector(mfield[0],  mfield[1], mfield[2]);
	index = (index + 1) & 1;
