...
int SecCamera::initCamera(int index)
{
    ...
    if (!m_flag_init) {
        ...
        ALOGD("%s: open(%s) --> m_cam_fd %d", __FUNCTION__, CAMERA_DEV_NAME, m_cam_fd);
        ...
        ALOGD("%s: open(%s) --> m_cam_fd2 = %d", __FUNCTION__, CAMERA_DEV_NAME2, m_cam_fd2);
        if (m_cam_fd2 < 0) {
...
int SecCamera::startPreview(void)
{
    ...
    ALOGD("%s: got the first frame of the preview\n", __func__);
