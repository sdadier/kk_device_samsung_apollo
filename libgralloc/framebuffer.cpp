...
static int fb_post(struct framebuffer_device_t* dev, buffer_handle_t buffer)
{
	...
	if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
		...
		m->base.lock(&m->base, buffer,
          ...
          ALOGD("Post done with memcopy : fb_vaddr %x buffer_vaddr %x", fb_vaddr, buffer_vaddr);
...
*/
#if 0
		s3c_g2d_copy_buffer(m->s3c_g2d_fd, buffer, (unsigned long)buffer_vaddr,
				m->framebuffer->fd, (unsigned long)fb_vaddr,
...
int mapFrameBufferLocked(struct private_module_t* module)
{
	...
	*/
	...
	module->framebuffer = new private_handle_t(dup(fd), fbSize,
			private_handle_t::PRIV_FLAGS_USES_PMEM, HAL_PIXEL_FORMAT_RGBA_8888, info.xres, info.yres);
